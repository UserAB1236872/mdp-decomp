...
class DQN(QLearn):
    ...
    def __init__(self, q_world, model, algo, mem_len=100000, swap_interval=10, batch_size=32, **kwargs):
    ...
    def observe(self, action, rewards, next_state, terminal):
        experience = Experience(self.world.statify(
            self.state).flatten(), action, rewards, self.world.statify(next_state).flatten(), terminal)
